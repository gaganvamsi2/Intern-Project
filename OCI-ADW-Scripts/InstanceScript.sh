...
#ssh opc@${COMPUTE_IP1} 'sudo yum -y update ; sudo yum -y update;sudo yum install -y docker; sudo docker pull gaganvamsi/tc_server:latest; sudo docker run -p8080:8080 gaganvamsi/tc_server:latest'
...
#ssh opc@${COMPUTE_IP2} 'sudo yum -y update ; sudo yum -y update;sudo yum install -y docker; sudo docker pull gaganvamsi/tc_server:latest; sudo docker run -p8080:8080 gaganvamsi/tc_server:latest'
...
#ssh opc@${COMPUTE_IP3} 'sudo yum -y update ;  sudo firewall-cmd --permanent --zone=public --add-service=http ; sudo firewall-cmd --reload; sudo yum -y update;sudo yum install -y docker;sudo service docker start; sudo docker pull gaganvamsi/tc_server:latest; sudo docker run -p8080:8080 gaganvamsi/tc_server:latest'